...
Venda.Attributes.UpdateDD = function (attName, uID) {
		...
		if (Venda.Attributes.productArr[i].attSet.id == uID) {
			...
			for (var t = 0; t < Venda.Attributes.productArr[i].attSet[attName].options.length; t++) {
				...
				if(stockstatus == 'In stock'){
					options += '<option value="'+ Venda.Attributes.productArr[i].attSet[attName].options[t] +'">' + Venda.Attributes.productArr[i].attSet[attName].options[t] + '<span class="hiddenStockStatus"> </span></option>';
				}else{
					options += '<option value="'+ Venda.Attributes.productArr[i].attSet[attName].options[t] +'">' + Venda.Attributes.productArr[i].attSet[attName].options[t] + '<span class="hiddenStockStatus"> - ' + stockstatus + '</span></option>';
				}
			}
	...
	if(attName == 'att2') stockStatusChange(); 
};