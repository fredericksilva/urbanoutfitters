...
jQuery("html.ui-mobile").live('pageinit',function(event){
	...
	jQuery('.enlarge').fadeIn(3000).fadeOut(5000);
	jQuery('#addedmsg1').fadeIn(3000).fadeOut(5000);
});
...
Venda.Attributes.Initialize = function() {
	...
	if (jQuery(jQuery("#alternativeImgID").text()).find("a").length > 1 && jQuery('#addedmsg1').length == 0) jQuery('.enlarge').fadeIn(3000).fadeOut(5000);
};
...
Venda.Attributes.StockStatus = function(stockAmount,theIndex) {
	...
	if(stockAmount > Venda.Attributes.Settings.lowStockThreshold && HasReleaseDate) return "Pre-order";
 	if(stockAmount <= Venda.Attributes.Settings.lowStockThreshold && stockAmount > jQuery("#OutOfStockThreshold").text()) return "Low stock";
...
Venda.Attributes.swatchImage = function(attnumber, uID){
	jQuery('#oneProduct_' + uID + ' #swatchList_'+attnumber+' li').each(function(index) {
		...
		var imgURL = jQuery("#tag-ebizurl").text() + '/content/ebiz/' + jQuery("#tag-ebizref").text() + '/invt/' + jQuery("#invtref").text() + '/' + jQuery("#invtref").text()+ '_' + swatchdata.toLowerCase() + '_sw.jpg';
		var me = this;
		// Image URL is validated. In case it is not valid, a default image will be added
		jQuery("<img>", {
			src: imgURL,
			error: function() {
				jQuery(me).hide();
			...
			load: function() {
				jQuery(me).css("background-image", "url(" + imgURL + ")");
			}
		});
	});
};
...
Venda.Attributes.ImageSwap = function(att) {
	...
	if(Venda.Attributes.storeImgsArr.length > 0) {
		...
		var associatedAttrCount = 0;
		jQuery.each(Venda.Attributes.storeImgsArr, function() {
			if (this.param != "") {
				associatedAttrCount++;
			}
		});
		/*if (associatedAttrCount != 1) {
			Venda.Attributes.ViewAlternativeImg(obj);
			Venda.Attributes.ViewMainImg(obj,0);
		} else {
			Venda.Attributes.ViewAlternativeImg(Venda.Attributes.storeImgsArr[0]);
			Venda.Attributes.ViewMainImg(Venda.Attributes.storeImgsArr[0],0);
		}*/
		Venda.Attributes.ViewAlternativeImg(obj);
		Venda.Attributes.ViewMainImg(obj,0);
	}
...
Venda.Attributes.ViewMainImg = function(obj, current) {
	...
	var mediumImg = "";
	 ...
	 }).error(function(){console.warn('Sorry an image cannot be loaded at the moment.'); objMedium.removeClass("preload");});
};
...
Venda.Attributes.ViewAlternativeImg = function(obj) {
	...
	objAltview.find("img").load(function(){
		jQuery(this).unbind("load").show();
	}).error(function(){
	...
	jQuery(".prodImages").live("swipeleft swiperight", function(e){
		...
		switch(e.type){
			case "swipeleft":
				...
				objAltview.find("a:visible").eq(current).trigger("click");
				return false;
			break;
