 ...
 *
 * This is the dropdown interface used to display select dropdown boxes for each attribute and autoselect att3
 *
...
Venda.Attributes.SetAtt3 = function (auID) {
  for(var i = 0; i < Venda.Attributes.attsArray.length; i++) {
    if(Venda.Attributes.attsArray[i].att1 == Venda.Attributes.productArr[0].attSet.att1.selected){
      var updatedAtt3 = Venda.Attributes.attsArray[i].att3;
      break;
    }
  } 
  if(updatedAtt3 != "") {
    Venda.Attributes.setSelectedJSON("att3",updatedAtt3,auID);
    Venda.Attributes.updateAttributes(auID);
  }
};