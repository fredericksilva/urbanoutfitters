...
$(function () {
	...
	jQuery('.showviewall').css('display','block');
	
	jQuery('#collate h3.collateheading').live('click',function(){
		if(jQuery("#"+this.id+"results").is(':visible')){
			jQuery("#"+this.id+"results").hide("fast");
			jQuery(this).css({backgroundPosition:'-1px -537px'});
		} else {
			jQuery("#"+this.id+"results").show("fast");
			jQuery(this).css({backgroundPosition:'-1px -521px'});
		};
	});
				
	jQuery(".collateresults .filter").live('keyup', function(){
		var filter=jQuery(this).val(), count=0;
		var collateField=this.id;
		collateField=collateField.replace("filter","");
		
		jQuery("."+collateField+"collateresult:first div").each(function(){
			if(jQuery(this).text().search(new RegExp(filter,"i"))<0) {
				jQuery(this).addClass("hide");
			} else {
				jQuery(this).removeClass("hide");
				var result=jQuery(this).find(".facet").text();
				var reg=new RegExp(filter,"gi");
				var final_str=result.replace(reg,function(str){return'<span class="highlight">'+str+'</span>'});
				jQuery(this).find(".facet").html(final_str);count++;
			}
		});
	});
	...
	/* CHECKBOX */
	$(".collateresults input.updatesearch:checkbox").live('change', function () {
		Venda.Widget.Search.scrollToResults();
		if ($(this).is(':checked')) {
			Venda.Widget.Search.UpdateURL(Venda.Widget.Search.EscapeAndFix($(this).val()), true);
		} else {
			Venda.Widget.Search.DeleteParams(Venda.Widget.Search.EscapeAndFix($(this).val()));
		}
	});
	/* end of CHECKBOX */
	
	/* SIMPLE A */
	$('.refinelist a.updatesearch').live('click', function (event) {
		Venda.Widget.Search.scrollToResults();
		event.preventDefault();
		Venda.Widget.Search.UpdateURL(Venda.Widget.Search.EscapeAndFix($(this).attr("rel")), true);
	});
	/* end of SIMPLE A */
	
	/* PAGINATION */
	$('.pagn a.updatesearch, .pagn a.viewProduct').live('click', function (event) {
		Venda.Widget.Search.scrollToResults();
	...
	/* FORM */
	$(".collatedresult form").live('submit', function (event) {
		event.preventDefault();
		var passedFields = 0;
		$("#" + this.id + ' input:text').each(function () {
			if ($(this).val() === "") {
				//$("#tag-rangevalidate").html();
				return;
			} else {
		...
		if (passedFields > 1) {
			//Venda.Widget.Search.load("/search?" + Venda.Widget.Search.vars.SOLRParentRef + $(this).serialize());
		}
	});
	/* end of FORM */
	
	/* SORT BY */
	$("#sortby, #perpagedpd").live('change', function () {
		Venda.Widget.Search.scrollToResults();
	...
	/* REMOVE SEARCH */
	$('.refinelist a.removesearch').live("click", function (event) {
		event.preventDefault();
		if(($(this).attr("rel").split('=')[0] == "q") || ($(this).attr("rel").split('=')[0] == "parentCategoryRef")) { location.href = "http://" + location.host + "/search?q=*"; } else {
			var preParamRemove = $(this).attr("rel").split('=')[0];
			var postParamRemove = escape($(this).attr("rel").split('=')[1].replace("&amp;", "&").replace(/\s/g, '+'));
	...
	$('#content-search').ajaxStart(function () {
...
Venda.Widget.Search.load = function (url, params) {
	...
	$('#content-search').load(url + ' #content-search', function (response, status, xhr) {
		...
		if (status === "error") {
			...
			$("#searchresults #response").html('<h2 class="error">' + $('#tag-error').text() + xhr.status + ' ' + xhr.statusText + '</h2>');
		} else {
			//ajax success - re-run all the JS that is triggered onload/ready - will change per site, this is for showcase
			//if ($('#tag-showviewall').text() === "1") {
				Venda.Widget.Search.viewAllRefinements();
				Venda.Widget.Search.createClass();
				Venda.Widget.Search.createSlider();
				Venda.Widget.ViewStyle.addClassForViewStyle("setView");
			//}
		}
...
Venda.Widget.Search.UpdateURL = function (newParams, pagnTrue) {
	...
	if((pagnTrue == true) && (isPagn >= 0)) {
		...
		$.history.load("!" + newPagnStr);
	} else {
		$.history.load("!" + Venda.Widget.Search.vars.refineParams);
	}
...
Venda.Widget.Search.scrollToResults = function () {
	if (!$('a#content:in-viewport').length) {
		$('html, body').animate({
...
Venda.Widget.Search.createSlider = function () {
	...
	var minprice = Math.floor(parseFloat($("#tag-minprice").text()));
	var maxprice = Math.ceil(parseFloat($("#tag-maxprice").text()));
	var from = Math.floor(parseFloat($("#tag-pricefrom").text()));
	var to = Math.ceil(parseFloat($("#tag-priceto").text()));
	var currency = $("#tag-currsym").text();
	$("#priceslider").slider({
			range : true,
			min : minprice,
			max : maxprice,
			...
			slide : function (event, ui) {
				$("#pricerangevalues").text(currency + ui.values[0] + " - " + currency + ui.values[1]);
			},
			stop : function (event, ui) {
				...
				Venda.Widget.Search.scrollToResults();
				Venda.Widget.Search.priceLoadFix( $("#pricerange").serialize());
			}
		});
	$("#pricerangevalues").text(currency + $("#priceslider").slider("values", 0) + ' - ' + currency + $("#priceslider").slider("values", 1));
};
Venda.Widget.Search.createClass = function () { //to add a valid class using field type and value when no ref available
	$(".refineImage div").each(function () {
			var facet = $(this).attr("title");
			var facetclass = facet.replace(/[^a-zA-Z0-9]+/g, '');
			$(this).addClass(facetclass.toLowerCase());
		});
};
Venda.Widget.Search.viewAllRefinements = function () {
	
	//toggles viewall/viewless refinement options using setting in A/E/D
	$('.showviewall').each(function (index) {
			var terms = $(this).find(".termtext");
			var viewallcount = $("#tag-viewallcount").html();
			if ($(terms).length > viewallcount) {
				$(this).find(".termtext:last-child").after($("#tag-viewalllink").html());
				$(this).find(".toggleviewall").addClass('toggleviewall' + index);
				$(terms).each(function (index) {
						var indexcount = viewallcount - 1;
						if (index > indexcount) {
							$(this).addClass('exceedsviewcount');
						}
					});
			}
			$(this).find(".exceedsviewcount").wrapAll('<div id="viewall" class="viewall' + index + '" style="display:none"></div>');
			$(".toggleviewall" + index).toggle(function () {
					$(".viewall" + index).show();
					$('.toggleviewall' + index).text($("#tag-viewlesstext").text());
				}, function () {
					$(".viewall" + index).hide();
					$('.toggleviewall' + index).text($("#tag-viewalllink").text());
				});
		});
};
...
catch(error){return hash;}},encoder:encodeURIComponent};var iframeWrapper={id:"__$_history",init:function(){var html='<iframe id="'+this.id+'" style="display:none" src="javascript:false;" />';$("body").prepend(html);return this;},_document:function(){return $("#"+this.id)[0].contentWindow.document;},put:function(hash){var doc=this._document();doc.open();doc.close();locationWrapper.put(hash,doc);},get:function(){return locationWrapper.get(this._document());}};function initObjects(options){options=$.extend({unescape:false},options||{});locationWrapper.encoder=encoder(options.unescape);function encoder(unescape_){if(unescape_===true){return function(hash){return hash;};}
...
$.extend(self,implementations[self.type]);$.history=self;})($);