(function() {
  ...
  jQuery.fn.imgg = function(options) {
    ...
    if (options != null) jQuery.extend(settings, options);
    ...
    el = jQuery('<div id="img_g"/>').css('background-image', "url(" + currentImage + ")");
    this.append(el);
    el.height(settings.height).width(settings.width).css('position', 'relative');
    while (i++ < _rows) {
      while (j++ < _cols) {
        el.append(jQuery('<div id=id_' + i + '_' + j + ' class="tile" />').css('left', w * (j - 1)).css('top', h * (i - 1)).css('background-position', (-(w * (j - 1))) + 'px ' + -(h * (i - 1)) + 'px'));
      }
      j = 0;
    }
    ...
    i = j = 0;
    while (i++ < _rows) {
      this.append(jQuery('<div class="gridY" />').css('top', y = y + h).width(settings.width));
    }
    while (j++ < _cols) {
      this.append(jQuery('<div class="gridX" />').css('left', x = x + w).height(settings.height));
    }
